...
filedir = os.path.join(current_directory, 'SPR-CNN/2fre4time_data')  # type the path of training datan=0
...
# checkpoint
filepath='CNN_UMi_3path_2fre_SNRminus10dB_200ep.keras'
...
# load model
CNN = tf.keras.models.load_model('CNN_UMi_3path_2fre_SNRminus10dB_200ep.keras')
...
class NmseCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        nmse = logs.get('val_loss')
        self.nmse_values.append(nmse)

# 定義 nmse_callback
nmse_callback = NmseCallback()

# 修改 callbacks_list，添加 nmse_callback
callbacks_list = [checkpoint, nmse_callback]

# 訓練模型
history = model.fit(H_train_noisy, H_train, epochs=epochs_num, batch_size=batch_size_num, callbacks=callbacks_list, verbose=2, shuffle=True, validation_split=0.1)

# 繪製 NMSE vs epoch 圖表
plt.plot(range(1, epochs_num+1), nmse_callback.nmse_values)
plt.xlabel('Epoch')
plt.ylabel('NMSE')
